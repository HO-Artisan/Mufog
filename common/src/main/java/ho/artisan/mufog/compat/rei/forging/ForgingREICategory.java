...
import ho.artisan.mufog.compat.rei.widgets.ForgedTipREIWidget;
...
public class ForgingREICategory implements DisplayCategory<ForgingREIDisplay> {
    private static final Identifier GUI_TEXTURE = MufogMod.genID("textures/gui/manage/forging_anvil.png");
    private static final Identifier GUI_TEXTURE_BLUEPRINT = MufogMod.genID("textures/gui/manage/forging_anvil_blueprint.png");
    ...
    public Text getTitle() {
        return Text.translatable("block.mufog.forging_anvil");
    }

    @Override
    public Renderer getIcon() {
        return EntryStacks.of(MufItems.FORGING_ANVIL.get());
    }
    ...
    public List<Widget> setupDisplay(ForgingREIDisplay display, Rectangle bounds) {
        ...
        if (flag) {
            ...
            widgets.add(Widgets.withTooltip(blueprint, Text.translatable("tip.mufog.manage.need_blueprint").formatted(Formatting.BLUE)));
            ...
            widgets.add(Widgets.withTooltip(checkMark, Text.translatable("tip.mufog.manage.no_need_blueprint").formatted(Formatting.GREEN)));
        }
        ...
        //添加绑定箭头组件位置的Tip
        widgets.add(Widgets.withTooltip(arrow, Text.translatable("tip.mufog.manage.hammertimes", display.getHammerTimes()).formatted(color)));
